 ...
 * @Project : joe
 * @Package : joe.elasticsearch.helper.factory.agg
 * @Description : SUM AggregationBuilder Factory
 * @date : 2018年09月30日 16:29
 */
@Component
@AggFactoryType(AggType.SUM)
public class SumAggFactory implements AggFactory {
    /**
     * @param agg 通用AGG模型
    ...
    public AggregationBuilder createAgg(GeneralAgg agg) {
        return AggregationBuilders.sum(agg.getAggName()).field(agg.getField());
    }

    @Override
    public Object parseAgg(GeneralAgg agg, Aggregations aggregations) {
        Sum sum = aggregations.get(agg.getAggName());
        return sum.getValue();
    }
}
